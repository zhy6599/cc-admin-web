        ...
        <template #body-cell-opt="props">
          <q-td :props="props" :auto-width="true">
            <q-btn
              ...
              type="a"
              target="_blank"
              :href="`#/path/design?id=${props.row.id}`"
              color="primary"
              icon="mdi-image-edit-outline"
            >
              <q-tooltip>路径规划</q-tooltip>
            </q-btn>
            ...
            <btn-del label="路径规划" @confirm="del(props.row)" />
          </q-td>
        </template>
      </q-table>
    </div>
      ...
      <q-form @submit="save" class="dialog_card column">
        ...
        <div class="col column">
          <div class="row q-col-gutter-x-md dialog_form q-pa-md">
            ...
            <div class="col-12">
              <h5>分类目录：</h5>
              <catalogselect :form.sync="form" type="MapPath" />
            </div>
          </div>
          <div class="q-pl-md">
            <q-chip dense>
              <q-avatar icon="bookmark" color="red" text-color="white" />单击开始画线，双击结束
            </q-chip>
          </div>
          <drawline :path="form.path" class="col" ref="drawLineCmp" />
        </div>
  ...
  data() {
    return {
      ...
      url: {
        ...
        copy:
          '/map/path/copy',
        delete: '/map/path/delete',
        deleteBatch: '/map/path/deleteBatch',
        exportXlsUrl: '/map/path/exportXls',
        importExcelUrl: '/map/path/importExcel',
      },
    };
  },
  methods: {
    ...
    editBefore(p) {
      // console.log(p.path);
      // if (p.path) {
      //   this.$refs.drawLineCmp.setLine(JSON.parse(p.path));
      // }
      return true;
    },
    save() {
      const lineArray = this.$refs.drawLineCmp.getLine();
      if (!lineArray || lineArray.length !== 1) {
        this.$error('只能画一条线');
        return;
      }
      const line = lineArray[0];
      this.form.path = JSON.stringify(line);
