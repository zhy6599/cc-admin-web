  ...
  <table v-if="treeData.name">
    <tr>
      <td
        ...
        :class="{parentLevel: Array.isArray(treeData.children) &&
        treeData.children.length, extend: Array.isArray(treeData.children) &&
        treeData.children.length && treeData.extend}"
      >
        <div :class="{node: true, hasMate: treeData.mate}">
          ...
          >
            <div class="avat" v-if="treeData.pic">
              <img :src="imagePath(treeData.pic)"
              style="width:50px;height:50px;border-radius:14px;"/>
            </div>
            <div class="name">
              <b class="q-mr-sm">{{ treeData.name }}</b>
              <span v-if="treeData.mate.name">({{ treeData.mate.name }})</span>
            </div>
          </div>
...
export default {
  ...
  watch: {
    json: {
      handler(Props) {
        const extendKey = function expFun(jsonData) {
