<template>
  <div class="cc-decoration-12 col" :ref="ref">
...
import { fade } from 'components/utils/color';
...
export default {
  ...
  methods: {
    init() {
      const {
        mergeColor, calcPathD, calcPathColor, calcCircleR, calcSplitLinePoints, calcArcD,
      } = this;
    ...
    calcPathD() {
      const {
        x, y, width, segment, sectorAngle, getCircleRadianPoint,
      } = this;
      ...
      this.pathD = new Array(segment)
        .fill('')
        .map((_, i) => {
          const endPoints = getCircleRadianPoint(x, y, r, startAngle - (i + 1) * angleGap)
            .map((n) => n.toFixed(5));
    ...
    calcSplitLinePoints() {
      const {
        x, y, width, getCircleRadianPoint,
      } = this;
