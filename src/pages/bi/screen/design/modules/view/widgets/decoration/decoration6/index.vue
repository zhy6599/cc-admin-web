<template>
  <div class="cc-decoration-6 col" :ref="ref">
    <svg
      :width="`${svgWH[0]}px`"
      :height="`${svgWH[1]}px`"
...
import { randomExtend } from 'components/utils/math';
...
export default {
  ...
  methods: {
    ...
    calcPointsPosition() {
      ...
      this.heights = new Array(rowNum * rowPoints)
        .fill(0).map(() => (Math.random() > 0.8
          ? randomExtend(0.7 * h, h) : randomExtend(0.2 * h, 0.5 * h)));
      this.minHeights = new Array(rowNum * rowPoints)
        .fill(0).map((foo, i) => this.heights[i] * Math.random());
